...
router.post('/admin/login', async (req, res) => {
    try {
        ...
        const token = jsonwebtoken_1.default.sign({ role: 'admin', username }, JWT_SECRET, { expiresIn: '8h' });
...
router.patch('/admin/contacts', requireAdmin, async (req, res) => {
    try {
        ...
        await db.collection('contacts').updateOne({ _id: new mongodb_1.ObjectId(id) }, { $set: { resolved: Boolean(resolved) } });
        res.json({ success: true });
    }
    catch (err) {
        console.error(err);
...
router.delete('/admin/events', requireAdmin, async (req, res) => {
    try {
        const { id } = req.query;
        if (!id)
        ...
        await db.collection('events').deleteOne({ _id: new mongodb_1.ObjectId(String(id)) });
        res.json({ success: true });
    }
    catch (err) {
        console.error(err);
...
router.delete('/admin/gallery', requireAdmin, async (req, res) => {
    try {
        const { id } = req.query;
        if (!id)
            return res.status(400).json({ error: 'Missing id' });
        ...
        await db.collection('gallery').deleteOne({ _id: new mongodb_1.ObjectId(String(id)) });
        res.json({ success: true });
    }
    catch (err) {
        console.error(err);
